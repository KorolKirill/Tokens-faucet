...
namespace eosio
{
   ...
   void token::addissuer(const name issuer, const symbol_code token_name)
   {
      ...
      stats statstable(get_self(), token_name.raw());
      auto existing = statstable.require_find(token_name.raw(), "");
      auto issuersArr = existing->issuers;
      issuersArr.push_back(issuer);

      statstable.modify(existing, get_self(), [&](auto &entry)
                        { entry.issuers = issuersArr; });
   }
   ...
   void token::issue(const name issuer, const name &to, const asset &quantity, const string &memo)
   {
      ...
      require_auth(issuer);
      bool canIssue = false;
      for (int i = 0; i < existing->issuers.size(); i++)
      {
         if (existing->issuers[i] == issuer)
         {
            canIssue = true;
            break;
         }
      ...
      check(canIssue, "tokens can only be issued to issuer accounts");
